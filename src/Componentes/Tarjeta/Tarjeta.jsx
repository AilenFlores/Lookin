...
const Tarjeta = ({ contenido, tipo, mediaType }) => {
  ...
  if (media === 'person') {
    return (
      <div className="rounded-md shadow flex flex-col overflow-hidden w-24 text-center">
        ...
        />
        <p className="text-xs mt-1 bg-white rounded-md">{contenido.name}</p>
      </div>
    );
  }
  ...
  <div
  className={`relative rounded-xl border border-purple-300 shadow-[0_4px_20px_rgba(0,0,0,0.5)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.7)] hover:scale-[1.03] transition-transform duration-300 ease-in-out flex flex-col overflow-hidden ${tamanioCard}`}
>
    {tipo === "grande" && (
    ...
    <div
      className={`flex flex-col justify-center flex-grow bg-gradient-to-b from-purple-100 via-white to-purple-50 ${tipo === "grande" ? "p-3" : "p-2"}`}
    >
      <Titulo
